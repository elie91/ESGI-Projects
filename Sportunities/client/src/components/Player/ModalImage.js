...
const ModalImage = ({ open, setOpen, player, setPlayer }) => {
  ...
  const onSubmit = async (data) => {
    ...
    actions.update({
      ...
      enqueueSnackbar(t("player.dialog.image.success"), { variant: "success" });
      setPlayer(prev => {
        return {
          ...prev,
          user: {
  ...
  return (
    <CustomDialog
      ...
      setOpen={setOpen}
      maxWidth="sm"
      title="player.dialog.image.title"
      textSubmitButton="player.dialog.image.submitButton"
      handleSubmit={handleSubmit}
      onSubmit={onSubmit}
      loading={loading}
    >
      <FileUpload
        defaultValue={(player && player.user.image) ? player.user.image : null}
        register={register}
        setValue={setValue}
        name="image"
        label="user.image"
