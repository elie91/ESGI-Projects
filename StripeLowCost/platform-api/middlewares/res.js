module.exports = (req, res, next) => {
  ...
  res.error = (error) => {
    ...
    if(!process.env.NODE_ENV || process.env.NODE_ENV === `dev`){
      if (!error.code) {
        return res.status(400).send({ type: `err301`, fields: ``, data: ``, message: `UnknownError`, err: error || ``});
      }
      else{
        const {code, type, fields, data, message, err} = error;
        return res.status(code).send({ type: type, fields: fields || ``, data: data || ``, message: message, err: err || ``});
      }
    }

    // remove message and error if we'r in prod ENV
    else {
      if (!error.code) {
